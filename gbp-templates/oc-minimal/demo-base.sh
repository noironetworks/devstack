...
SERVICE_MANAGEMENT_ACCESS_RULE_SET_ID=`heat output-show "$CONTRACTS_STACK" "service_management_access_rule_set_id" | sed "s/\"//g"`

heat stack-create -f "$ADMIN_YAML" "$ADMIN_STACK" -P "web_access_prs_id=$HTTP_RULE_SET_ID;infra_external_segment_name=$INFRA_EXTERNAL_SEGMENT_NAME;infra_external_policy_name=$INFRA_EXTERNAL_POLICY_NAME;app_external_segment_name=$APP_EXTERNAL_SEGMENT_NAME;app_external_policy_name=$APP_EXTERNAL_POLICY_NAME;port_address_translation=$PORT_ADDRESS_TRANSLATION;infra_ip_pool=$INFRA_IP_POOL;app_ip_pool=$APP_IP_POOL;monitoring_rule_set_id=$MONITORING_RULE_SET_ID;monitoring_to_outside_rule_set_id=$MONITORING_TO_OUTSIDE_RULE_SET_ID;app_to_outside_rule_set_id=$APP_TO_OUTSIDE_RULE_SET_ID;service_management_access_rule_set_id=$SERVICE_MANAGEMENT_ACCESS_RULE_SET_ID;mysql_rule_set_id=$MYSQL_RULE_SET_ID;app_rule_set_id=$APP_RULE_SET_ID;web_tier_consumed_prs_id=$APP_TO_OUTSIDE_RULE_SET_ID;web_tier_provided_prs_id=$HTTP_WITH_LB_REDIRECT_RULE_SET_ID;monitoring_ptg_name=$MONITORING_PTG_NAME;service_management_ptg_name=$SERVICE_MANAGEMENT_PTG_NAME;management_ptg_name=$MANAGEMENT_PTG_NAME;monitoring_vm_image=$MONITORING_VM_IMAGE_NAME;monitoring_vm_flavor=$MONITORING_VM_FLAVOR;mgmt_vm_image=$MGMT_VM_IMAGE_NAME;mgmt_vm_flavor=$MGMT_VM_FLAVOR;svc_mgmt_vm_image=$SVC_MGMT_VM_IMAGE_NAME;svc_mgmt_vm_flavor=$SVC_MGMT_VM_FLAVOR;infra_l3_policy_name=$INFRA_L3_POLICY_NAME;app_l3_policy_name=$APP_L3_POLICY_NAME"
