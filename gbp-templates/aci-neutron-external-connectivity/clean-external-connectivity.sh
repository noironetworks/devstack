...
function delete_external_policies {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(gbp external-policy-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
        gbp external-policy-delete $id
        set +o xtrace
    done
    $xtrace
}

function delete_neutron_subnets {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(neutron subnet-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
...
function delete_neutron_networks {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(neutron net-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
...
function delete_neutron_routers {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(neutron router-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
...
function delete_policy_target_groups {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(gbp policy-target-group-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
        gbp policy-target-group-delete $id
        set +o xtrace
    done
    $xtrace
}

function delete_l2_policies {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(gbp l2policy-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
        gbp l2policy-delete $id
        set +o xtrace
    done
    $xtrace
}

function delete_l3_policies {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(gbp l3policy-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
        gbp l3policy-delete $id
        set +o xtrace
    done
    $xtrace
}

function delete_policy_rule_sets {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(gbp policy-rule-set-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
        gbp policy-rule-set-delete $id
        set +o xtrace
    done
    $xtrace
}

function delete_policy_rules {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(gbp policy-rule-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
        gbp policy-rule-delete $id
        set +o xtrace
    done
    $xtrace
}

function delete_policy_classifiers {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(gbp policy-classifier-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
        gbp policy-classifier-delete $id
        set +o xtrace
    done
    $xtrace
}

function delete_policy_actions {
    local xtrace=$(set +o | grep xtrace)
    set +o xtrace
    uuids="$(echo $(gbp policy-action-list | get_field 1) | sed 's/id //g')"
    for id in $uuids
    do
        set -o xtrace
        gbp policy-action-delete $id
        set +o xtrace
    done
    $xtrace
}
...
delete_policy_target_groups
delete_external_policies
delete_l2_policies
delete_l3_policies
delete_policy_rule_sets
delete_policy_rules
delete_policy_classifiers
delete_policy_actions
delete_external_segments
delete_neutron_subnets
delete_neutron_networks
delete_neutron_routers
